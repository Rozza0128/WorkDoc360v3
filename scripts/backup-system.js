...
class BackupSystem {
  ...
  async compressBackup(backupPath) {
    ...
    try {
      ...
      const backupBaseName = path.basename(resolvedBackupPath);
      
      // Ensure the backup path is within expected directory structure
      if (!resolvedBackupPath.startsWith(resolvedBackupDir)) {
        throw new Error('Invalid backup path: outside of backup directory');
      }
      
      // Use safe command execution to prevent injection
      const zipProcess = spawn('zip', ['-r', zipName, backupBaseName], {
        cwd: this.backupDir,
        stdio: 'inherit'
      });
