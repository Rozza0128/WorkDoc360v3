...
export function DocumentsTab({ companyId, companyName, tradeType, userRole }: DocumentsTabProps) {
  ...
  const filteredDocuments = (documents as GeneratedDocument[]).filter((doc: GeneratedDocument) => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "generating":
        return <Wand2 className="h-4 w-4 text-construction-orange animate-pulse" />;
      case "template":
        return <FileText className="h-4 w-4 text-gray-400" />;
      default:
        return <CheckCircle className="h-4 w-4 text-green-500" />; // Default to generated for any unknown status
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    ...
    // Normalize status display - show all completed documents as "Ready"
    const displayStatus = status === "generated" ? "Ready" : 
                  ...
                  <div className="flex items-center space-x-2 pt-3">
                    ...
                    <div className="flex-1 flex space-x-1">
                      <Button 
                        ...
                        onClick={async () => {
                          try {
                            ...
                            // Use fetch with proper credentials and blob handling
                            const downloadUrl = `/api/documents/${document.id}/download/pdf`;
                            ...
                            // Create download link that works in all browsers
                            ...
                            // Fallback: try direct link download for Chrome compatibility
                            try {
                              const fallbackUrl = `/api/documents/${document.id}/download/pdf`;
                              const fallbackWindow = window.open(fallbackUrl, '_blank');
                              if (!fallbackWindow) {
                                throw new Error('Popup blocked - please allow popups for downloads');
                              }
                              
                              toast({
                                title: "PDF Download",
                                description: "Opening download in new tab (fallback method)",
                              });
                            } catch (fallbackError) {
                              const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
                              toast({
                                title: "Download Failed",
                                description: `Unable to download PDF: ${errorMessage}`,
                                variant: "destructive"
                              });
                            }
                          }
                      ...
                      <Button 
                        ...
                        onClick={async () => {
                          try {
                            console.log('Starting Word download for document:', document.id);
                            
                            // Use fetch with proper credentials and blob handling
                            const downloadUrl = `/api/documents/${document.id}/download/word`;
                            const response = await fetch(downloadUrl, {
                              method: 'GET',
                              credentials: 'include', // Include cookies for authentication
                              headers: {
                            ...
                            console.error('Word download error details:', error);
                            
                            // Fallback: try direct link download for Chrome compatibility
                            try {
                              const fallbackUrl = `/api/documents/${document.id}/download/word`;
                              const fallbackWindow = window.open(fallbackUrl, '_blank');
                              if (!fallbackWindow) {
                                throw new Error('Popup blocked - please allow popups for downloads');
                              }
                              ...
                              toast({
                                title: "Download Failed",
                                description: `Unable to download Word: ${errorMessage}`,
                                variant: "destructive"
                              });
                            }
                          }
          ...
          <CardContent className="p-4">
            <div className="flex flex-wrap items-center justify-between gap-4 text-sm text-steel-gray">
              <div className="flex items-center space-x-6">
                ...
                <span>Generated: {filteredDocuments.filter((d: GeneratedDocument) => d.status === "generated").length}</span>
                <span>Drafts: {filteredDocuments.filter((d: GeneratedDocument) => d.status === "draft").length}</span>
              </div>
              <div className="flex items-center space-x-2">
